...
var promiseLogger = new IntermediateLogger(common.ACTUAL_OUTPUT_DIR);
...
(function (sandbox) {
    function MyAnalysis() {
        ...
        function isPromise (obj) {
            // return (obj === Promise || obj instanceof Promise || obj instanceof BluebirdPromise);
            if (typeof obj !== 'undefined') {
                return (obj === Promise || obj instanceof Promise || obj instanceof BluebirdPromise || obj === BluebirdPromise || getName(obj) === 'Promise');
            } else
                return false;
        }
        ...
        this.invokeFunPre = function (iid, f, base, args, isConstructor, isMethod, functionIid) {
            ...
            if (typeof f !== 'undefined' && getName(f) === 'then') {
                console.log('========= ^^^^^^^^^^^^');
            }

            if (isAnyBluebirdPromise(base)) {
                if (isBluebirdPromiseThen(base, f)) {
                    console.log('========= %%%%%%%%%%%%%%%%');
                }
                /*
                if (f === base.then) {
                ...
                if (f === BluebirdPromise.then) {
                    console.log('3&&&&&&&&&&&&&&&&&&&&&&');
                }
                if (f === BluebirdPromise.prototype.then) {
                    console.log('4&&&&&&&&&&&&&&&&&&&&&&');
                }
                // if (f === base.then || f === base._then || f === BluebirdPromise.then || f === BluebirdPromise.prototype.then) {
                //     console.log('&&&&&&&&&&&&&&&&&&&&&&');
                // }
                */
            }
            ...
            if (f === Function.prototype.bind) {
                if (isPromiseThen(base)) {
                    if (isPromise(args[0])) { // todo oooooo
                        // if (args[0] instanceof Promise) {
                        console.log('========= <<<<<< bind then on promise');
            ...
            // todo ooooo oooooooooooooo ==============
            if (isConstructor && (isPromise(f) || isAnyBluebirdPromise())) {
            ...
            if (f === BluebirdPromise.prototype.then) {
                console.log('========= 1_____________');
            }
            if (f === Promise.prototype.then) {
                console.log('========= 2_____________');
            }
            if (getName(f) === 'then') {
        ...
        this.endExecution = function () {
            ...
            var firstFileName = getFirstScript();
            ...
            if (firstFileName.indexOf('tests-nodejs') > -1) {  // nodejs tests
                ...
                var dirName = path.basename(path.dirname(firstFileName));
                var dirPath = path.join(path.dirname(firstFileName), '..');
                var prjName = dirPath.split(path.sep).pop();
                dirPath = path.join(dirPath, '../..');

                var textLogPath = path.join(dirPath, 'output-actual' + path.sep + 'text-logs', prjName, path.sep, path.parse(fileName).name + '_out.js');
                var jsonLogPath = path.join(dirPath, 'output-actual' + path.sep + 'json-logs', prjName, path.sep, fileName + '.json');

                promiseLogger.endExecution(appName, '', textLogPath, jsonLogPath);
                var promiseGraph = new PromisesGraph(fileName);

                var graphLogPath = path.join(dirPath, path.sep, 'output-actual/graphs', path.sep, prjName);
                var graphVisLogPath = path.join(dirPath, path.sep, 'output-actual/graph-vis', path.sep, prjName);

                promiseGraph.importLog(fileName, path.join(dirPath, 'output-actual/json-logs', prjName),
        ...
        function getFirstScript () {
            var firstScript = __originalFileNames[0];
            console.log('%%%%% ' + __originalFileNames[0]);
            console.log('%%%%% ' + __originalFileNames);

            if (typeof currentTestName !== 'undefined' && currentTestName.length > 0) {
                console.log('%%% ' + currentTestName);
                var testNameIndex = firstScript.indexOf('__run-tests');
                if (testNameIndex > -1) {
                    var testPath = firstScript.slice(0, testNameIndex - 1);
                    firstScript = path.join(testPath, currentTestName + '.js');
                }
            }
            return firstScript;
        }
